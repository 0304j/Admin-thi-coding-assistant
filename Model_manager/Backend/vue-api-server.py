...
Provides REST endpoints for the Vue.js frontend
+ Model Usage Tracking & Ranking (Persistent via ConfigMap)
"""
...
from Backend.k8s_client import KubernetesClient
...
# ------------------------------------------------------------
# Model Usage Tracking System (Persistent via ConfigMap)
# ------------------------------------------------------------
usage_lock = Lock()
...
def load_usage_from_configmap():
    """Load usage stats from Kubernetes ConfigMap"""
    try:
        ...
        v1 = client.CoreV1Api()
        try:
            cm = v1.read_namespaced_config_map(USAGE_CONFIGMAP_NAME, USAGE_NAMESPACE)
            if cm.data and "usage_stats" in cm.data:
                data = json.loads(cm.data["usage_stats"])
                logger.info(f"[USAGE] Loaded {len(data)} models from ConfigMap")
                return defaultdict(lambda: {"count": 0}, {k: {"count": v} for k, v in data.items()})
        except client.exceptions.ApiException as e:
            if e.status == 404:
                logger.info("[USAGE] ConfigMap not found, creating new one")
                # Create the ConfigMap
                cm = client.V1ConfigMap(
                    metadata=client.V1ObjectMeta(name=USAGE_CONFIGMAP_NAME),
                    data={"usage_stats": "{}"}
                )
                v1.create_namespaced_config_map(USAGE_NAMESPACE, cm)
            else:
                logger.warning(f"[USAGE] Error reading ConfigMap: {e}")
    except Exception as e:
        logger.warning(f"[USAGE] Could not load from ConfigMap: {e}")
    
    return defaultdict(lambda: {"count": 0})

def save_usage_to_configmap():
    """Save usage stats to Kubernetes ConfigMap"""
    try:
        from kubernetes import client, config
        try:
            config.load_incluster_config()
        ...
        simple_data = {k: v["count"] for k, v in model_usage_stats.items()}
        
        try:
            cm = v1.read_namespaced_config_map(USAGE_CONFIGMAP_NAME, USAGE_NAMESPACE)
            cm.data = {"usage_stats": json.dumps(simple_data)}
            v1.replace_namespaced_config_map(USAGE_CONFIGMAP_NAME, USAGE_NAMESPACE, cm)
            logger.debug(f"[USAGE] Saved {len(simple_data)} models to ConfigMap")
        except client.exceptions.ApiException as e:
            if e.status == 404:
                cm = client.V1ConfigMap(
                    metadata=client.V1ObjectMeta(name=USAGE_CONFIGMAP_NAME),
                    data={"usage_stats": json.dumps(simple_data)}
                )
                v1.create_namespaced_config_map(USAGE_NAMESPACE, cm)
                logger.info("[USAGE] Created new ConfigMap with usage stats")
    except Exception as e:
        logger.warning(f"[USAGE] Could not save to ConfigMap: {e}")
...
def track_model_usage(model_name: str):
    """Track model usage for ranking - persists to ConfigMap"""
    with usage_lock:
        model_usage_stats[model_name]["count"] += 1
        count = model_usage_stats[model_name]["count"]
        logger.info(f"[USAGE] Tracked: {model_name}, count: {count}")
        # Save to ConfigMap after each update
...
def health():
    ...
    if k8s_client and k8s_client.connected:
        return jsonify({"status": "healthy", "kubernetes": "connected"})
    else:
        return jsonify({"status": "unhealthy", "kubernetes": "disconnected"}), 503
...
def pull_ollama_model():
    """Pull a new model to Ollama with progress tracking"""
    try:
        data = request.json
                                ...
                                try:
                                    ...
                                    with pull_progress_lock:
                                        ...
                                        if total > 0:
                                            ...
                                            percent = pull_progress[model_name].get('progress', 0)
                                        ...
                                        pull_progress[model_name] = {
                                            ...
                                            'completed': completed,
                                            'error': None,
                                            'message': f"{status}: {percent}%" if total > 0 else status
                                        }
                                        
                                        logger.debug(f"[PULL] {model_name}: {status} - {percent}%")
        ...
        # Return immediately
        return jsonify({
            'message': f'Pull started for model "{model_name}".',
            'model': model_name
        }), 202
    ...
    try:
        with pull_progress_lock:
            if model_name in pull_progress:
                progress = pull_progress[model_name].copy()
                # Clean up completed/failed pulls after they're retrieved
                if progress['status'] in ['success', 'error']:
                    # Keep it for a bit so frontend can get final status
                    pass
                ...
                return jsonify({
                    ...
                    'completed': 0,
                    'error': None,
                    'message': 'No pull in progress for this model'
                })
    except Exception as e:
        logger.error(f"Error getting pull progress: {e}")
        return jsonify({'error': str(e)}), 500

@app.route('/api/ollama/pull/progress', methods=['GET'])
def get_all_pull_progress():
    """Get progress of all active pull operations"""
    try:
        with pull_progress_lock:
            return jsonify(dict(pull_progress))
    except Exception as e:
        logger.error(f"Error getting all pull progress: {e}")
        return jsonify({'error': str(e)}), 500
...
def test_ollama_model():
    """Test model generation with Ollama - tracks usage for ranking"""
    try:
        data = request.json
        model_name = data.get('model')
        ...
        for url in ollama_urls:
            try:
                ...
                if response.status_code == 200:
                    # Track usage when backend communicates with Ollama
                    track_model_usage(model_name)
...
# ------------------------------------------------------------
# Model Usage Tracking Endpoints
# ------------------------------------------------------------

@app.route('/api/usage/ranking', methods=['GET'])
def get_usage_ranking():
    """Get model usage ranking with percentages - simple version"""
    with usage_lock:
        # Calculate total requests
        total_requests = sum(stats["count"] for stats in model_usage_stats.values())
        ...
        # Build simple ranking with only rank, model, count, percentage
        ranking = []
        for rank, (model_name, stats) in enumerate(sorted_models, 1):
            usage_percent = (stats["count"] / total_requests * 100)
            ranking.append({
                'rank': rank,
                'model': model_name,
                'count': stats["count"],
...
def track_usage_endpoint():
    """Manually track a model usage (for external calls)"""
    try:
        data = request.json
        model_name = data.get('model')
        ...
        return jsonify({
            'success': True,
            'message': f'Usage tracked for {model_name}',
            'current_count': model_usage_stats[model_name]["count"]
        })
    except Exception as e:
        return jsonify({'error': str(e)}), 500
...
def reset_usage_stats():
    """Reset all usage statistics and clear ConfigMap"""
    global model_usage_stats
    with usage_lock:
        model_usage_stats = defaultdict(lambda: {"count": 0})
        # Clear the ConfigMap too
        save_usage_to_configmap()
